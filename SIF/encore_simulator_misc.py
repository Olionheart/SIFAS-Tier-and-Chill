...
if __name__ == "__main__":
    ...
    for n in range(20,31):
        score_note = [n]
        score_mag = [[a * n / 25 for a in [4435, 4875, 5760, 7090, 8860, 9370, 10385, 11915, 13955]]] # list of list of magnitude of each scorer
        simulated_score = list()
        for simulation_round in range(1000):
            simulated_score.append(simulate(spell_queue) + tap_score)
        mean_score.append(np.mean(simulated_score))
        high_score.append(np.percentile(simulated_score, 99))

    x = [i for i in range(20,31)]
    plt.plot(x,mean_score,'b',x,high_score,'r')
    plt.legend(['mean score','99-th percentile score'])
    plt.xlabel('scorer note')
    plt.ylabel('total score')
    plt.title('effect of scorer activation note on 25n encore team')
    plt.show()
